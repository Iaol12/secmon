...
class FilterController extends Controller
{
    ...
    public function actionGetEventsBarChart($filterId = null, $timeFilterId = null)
    {
        $this->checkAccess();

        $filter = !empty($filterId) ? Filter::findOne($filterId) : null;
        $timeFilter = !empty($timeFilterId) ? Filter::findOne($timeFilterId) : null;
        ...
        if (!empty($timeFilter) && $timeFilter->user_id != Yii::$app->user->getId()) {
            throw new ForbiddenHttpException('You do not have permission to access this time filter.');
        }
        ...
        if ($eventsCount > 1) {
            if (!empty($timeFilter)) {
                // Get time interval from filter rules
                $timeUnit = $this->determineTimeUnit($timeFilter);
            }
        }
